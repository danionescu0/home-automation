...
class SensorsRepository(AbstractRepository):
    ...
    def get_sensor_values(self, id: str, start_date: datetime, end_date: datetime) -> List[SensorDataPoint]:
        ...
        data = self.client.zrangebyscore(redis_key, start_timestamp, end_timestamp, withscores=True)

        return [SensorDataPoint(json.loads(datapoint[0].decode("utf-8"))['value'], datapoint[1]) for datapoint in data]
    ...
    def __get_sensor_properties(self, properties: dict) -> SensorProperties:
        actuator_properties = SensorProperties()
