...
communicator_registry = container.communicator_registry()
...
communicator_registry.configure_communicators()
async_jobs.connect()
...
def main():
    ...
    threads.append(IncommingCommunicationThread(container.text_sensor_data_parser(),
                                                ...
                                                communicator_registry.get_communicator('bluetooth'), root_logger))
    threads.append(IncommingCommunicationThread(container.text_sensor_data_parser(), sensors_repo,
                                                container.sensor_update_event(),
                                                communicator_registry.get_communicator('serial'), root_logger))
    threads.append(AsyncJobsThread(async_jobs, container.change_actuator_request_event(), root_logger))
